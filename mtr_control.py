...
save_flag = False # Chose to save output images
...
SP_amplitude = 10.0; # not time varying
...
# SP_amplitude = 1.0*60/(2*np.pi); # not time varying
# SP_period      = np.pi; # pi means 1 cycle every 2 seconds
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

# Controller parameters
V_bias    = 0.0       # Used for constant controller output
k_p       = 0.03          # Will depend on the variable we are controlling
T_u       = 0.03         # oscillation period for tuning
T_i       = 0*T_u/2.0;  # allow tuning the time constants, but basically a function of T_u
T_d       = T_u/8.0;
...
Time      =  10.0;  # Final time for simulation (Adjust this to see detailed start or longer for tracking)
...
mtr_simulation_plots(tv,SPv,Pv,Tv,wv,iv,vv,ev,intv,Vtitle,Vname,save_flag)
